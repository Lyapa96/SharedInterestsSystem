    ...
    * The following properties are assigned as a node or edge attribute
    ...
    def __init__(self, g, q_classes=None, q_args=None, seed=None, colors=None,
        ...
        if g is not None:
            ...
            # Считаем вероятности перехода по каждому ребру
            for v in g.nodes():
                vod = g.out_degree(v)
    ...
    def _simulate_next_event(self, slow=True):
        ...
        q1k = self._fancy_heap.pop() # queue 1 key
        q1t = q1k[0] # queue 1 time of next event
        q1 = self.edge2queue[q1k[1]]
        e1 = q1.edge[2] # edge index
        ...
        if event == 2:  # This is a departure
            ...
            if q2.at_capacity() and e2 != e1: # Можно ли направить в другую очередь агента - если не влазит
                q2.num_blocked += 1
                ...
                agent = q1.next_event() # Т.к событие - убытие - вернется агент
                agent._time = q1t
            ...
            if new_q2k[0] != q2k[0]: # Если после добавления агента время следующего события изменилось
                self._fancy_heap.push(*new_q2k)

                if new_q1k[0] < np.infty and new_q1k != new_q2k:
                    self._fancy_heap.push(*new_q1k)
            else: # Время не изменилось - агент попал в очередь в q2
            ...
            new_q1k = q1._key() # получили следующее событие на очереди
            if new_q1k[0] < np.infty:
                self._fancy_heap.push(*new_q1k)
