...
class QueueServer(object):
    ...
    def _add_arrival(self, agent=None):
        if agent is not None:
            ...
            if self._current_t >= self._next_ct: # Проверочное на логику условие
                ...
                self._num_total += 1
                new_agent = self.AgentFactory((self.edge[2], self._oArrivals)) # Ставим agent_id
        ...
        if self._arrivals[0]._time < self._departures[0]._time: # После доб агента пересчитать время след события
    ...
    def next_event(self):
        ...
        if self._departures[0]._time < self._arrivals[0]._time:
            ...
            if len(self.queue) > 0: # Т.к агент ушел можно взять из очереди ожидания на обслуживание
            ...
            if self.num_system <= self.num_servers: # Если есть свободный сервер - получаем время окончания обслуж
                if self.collect_data:
                    self.data[arrival.agent_id][-1][1] = arrival._time

                arrival._time = self.service_f(arrival._time) # До этого в агенте _time - время прибытия
